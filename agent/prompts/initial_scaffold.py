...
INITIAL_SCAFFOLD_PROMPT = (
    ...
    "Call the `run_shell` tool with `ls -R {{project_subdirectory}}` to understand the new file structure. "
    "After exploring:\n- **Verify the project builds** by calling `run_shell` with `npm run build {{project_subdirectory}}`.\n"
    "- **Do NOT** run `npm run dev` or start any long-running development servers; they will not exit and will break the automation.\n\n"
    "- After the build check, re-examine the original user prompt. If the prompt implies further modifications (e.g., 'Make a hello world app' implies changing the page content), proceed with those modifications. Do not stop and ask the user what to do next if the original prompt contains more instructions.\n\n"
