...
-   [ ] **1. LangGraph Agent - Initial Setup:**
    ...
    -   [x] Details:
        ...
        -   [ ] The "planner_llm_step" node will:
            ...
            -   [x] Make a **simple, real LLM call** (OpenAI).
            -   [ ] **(Revised)** The LLM's first response should be a tool call to `run_shell` with the `npx create-next-app...` command.
...
-   [ ] **3. Tool Definition & Initial Implementations (`tools/` directory):**
    -   [x] General: Add `langchain-community`, `openai`, `chromadb`, `unidiff`, `GitPython`, `mcp`, `python-multipart` to `requirements.txt`.
